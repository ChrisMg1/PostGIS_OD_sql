...
def bathtub2 (x_in, l, r, a1l, a1r, transp):
...
def bathtub3 (x_in, l, r, a1l, a1r, transp):
    if (x_in > 2.0 * r):   # avoid out-of-range errors; ONLY FOR 'U'-SHAPE !!! threshold seems sufficient for bathtub, not for logit
        return 1.0
    elif (x_in < ((l + r) * transp)):
        return 1.0-(1.0 / (1.0 + 25.0*(np.exp(-a1l * (x_in - l) )) ))
    elif (x_in >= ((l + r) * transp)):
        return (1.0 / (1.0 +        (np.exp(-a1r * (x_in - r) )) ))
    else:
        raise ValueError("CM: Invalid Thresholds etc.")
...
for i in x_dist:
    bathtub_vals.append(bathtub2(i, shift_left_dist, shift_right_dist, a1l_in_dist, a1r_in_dist, transition_point))
...
for i in x_PAX:
    PAX_vals2.append(bathtub2(i, shift_left_PAX, shift_right_PAX, a1l_in_pax, a1r_in_pax, transition_point))
...
for i in x_thresh:
    demand_threshold_bathtub.append(bathtub2(i, shift_left_demand, shift_right_demand, a1l_in_demand, a1r_in_demand, transition_point))
...
print('(4.3) With minimum, playing around (bathtub3)')

## Set parameters
shift_left_demand = 150.0 # Approx. 1 flight per hour: 24*4=96 _OR_ 12 flights per day: 12*4=48
shift_right_demand = 768.0 # 32.0*24.0=768: According paper Lukas, 32 PAX per hour
a1l_in_demand = 0.02
a1r_in_demand = 0.1
transition_point = 0.75 # Transition from left-hand (logit) function to right-hand (logit) function

demand_threshold_bathtub = []

for i in x_thresh:
    demand_threshold_bathtub.append(bathtub3(i, shift_left_demand, shift_right_demand, a1l_in_demand, a1r_in_demand, transition_point))
